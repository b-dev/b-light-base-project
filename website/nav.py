...
class Node(object):
    ...
    def __init__(self, label, url_name=None, url_args=None, url_kwargs=None, access_fn=None, icon=None):
        self.label = label
        self.icon = icon
...
def get_nodes(user, area):
    """
    Return the visible navigation nodes for the passed user
    """
    if area:
        settings_name = 'DASHBOARD_NAVIGATION_%s' % area.upper()
    else:
        settings_name = 'DASHBOARD_NAVIGATION'
    all_nodes = create_menu(getattr(settings, settings_name))
    visible_nodes = []
